...
const HeroBanner = ({ id, title, video_url, link_url }: HeroBannerProps) => {
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-2xl aspect-video">
      ...
      <div className="absolute top-[80%] left-0 right-0 z-10 flex justify-center -translate-y-1/2">
        {link_url ? (
          <a
            href={link_url}
            target="_blank"
            ...
            onClick={handleClick}
            className="block w-[70%] max-w-xl transition-transform hover:scale-[1.02] active:scale-[0.98]"
          >
            <img
              src={btnEntrarAgora}
              alt="Entrar Agora"
