...
function buildSitemapXml(
  ...
  privacyModels: { id: number; name: string }[]
) {
  ...
  const modelUrls = privacyModels
    ...
    .map((model) => {
      ...
      const urlPath = slug ? `/modelo/${slug}-${model.id}` : `/modelo/${model.id}`;
      return `  <url>
    <loc>${BASE_URL}${urlPath}</loc>
    <lastmod>${today}</lastmod>
    <changefreq>weekly</changefreq>
...
function sitemapPlugin(): Plugin {
  return {
    ...
    async buildStart() {
      try {
        console.log("[sitemap] Fetching groups and privacy models...");
        const [groups, privacyModels] = await Promise.all([
          fetchAllGroups(),
          fetchAllPrivacyModels(),
        ]);
        console.log(`[sitemap] Fetched ${groups.length} groups, ${privacyModels.length} privacy models.`);
        const xml = buildSitemapXml(groups, privacyModels);
        writeFileSync("public/sitemap.xml", xml, "utf-8");
        console.log(`[sitemap] Generated public/sitemap.xml with ${STATIC_PAGES.length + groups.length + privacyModels.length} total URLs.`);
      } catch (e) {
